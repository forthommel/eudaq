...
//  float vthresh_values[TLU_PMTS];
  ...
  eudaq::Option<std::string> ConnectionFile(op, "cf", "connectionFile", "file://connection.xml", "filename", "IPBus/uHAL connection file describing network connection to TLU");
  eudaq::Option<std::string> DeviceName(op, "dn", "deviceName", "caliceahcalbif", "filename", "IPBus/uHAL connection file describing network connection to TLU");
  eudaq::Option<int> trigg(op, "t", "trigger", 0, "msecs", "The interval in milliseconds for internally generated triggers (0 = off)");
  eudaq::Option<int> hsmode(op, "hm", "handshakemode", 0x3F, "nohandshake", "Bit mask for handshake mode. Set to zero and in this mode the TLU issues a fixed-length pulse on the trigger line (0 = no hand shake)");
  eudaq::Option<int> dmask(op, "d", "dutmask", 0, "mask", "The mask for enabling the DUT connections");
  eudaq::Option<int> vmask(op, "v", "vetomask", 0, "mask", "The mask for vetoing external triggers");
  eudaq::Option<int> amask(op, "a", "andmask", 255, "mask", "The mask for coincidence of external triggers");
  eudaq::Option<int> omask(op, "o", "ormask", 0, "mask", "The mask for ORing of external triggers");

  eudaq::Option<int> wait(op, "w", "wait", 1000, "ms", "Time to wait between updates in milliseconds");
  eudaq::Option<int> strobeperiod(op, "p", "strobeperiod", 1000, "cycles", "Period for timing strobe in clock cycles");
  eudaq::Option<int> strobelength(op, "l", "strobelength", 100, "cycles", "Length of 'on' time for timing strobe in clock cycles");
  eudaq::Option<int> enabledutveto(op, "b", "dutveto", 0, "mask", "Mask for enabling veto of triggers ('backpressure') by rasing DUT_CLK");
  ...
  eudaq::Option<std::string> sname(op, "s", "save-file", "", "filename", "The filename to save trigger numbers and timestamps");
  ...
  try {
    ...
    if (vthresh_1.Value() != -1) {
      vthresh_values[0] = vthresh_1.Value();
    }
    if (vthresh_2.Value() != -1) {
      vthresh_values[1] = vthresh_2.Value();
    }
    if (vthresh_3.Value() != -1) {
      vthresh_values[2] = vthresh_3.Value();
    }
    if (vthresh_4.Value() != -1) {
      vthresh_values[3] = vthresh_4.Value();
    }*/
    ...
    << "Hand shake mode = " << hsmode.Value() << "\n" << "Trigger interval = " << trigg.Value() << "\n" << (trigg.Value() > 0 ? " ms (" + to_string(1e3 / trigg.Value()) + " Hz)" : std::string()) << "\n" << "DUT Mask  = "
        << hexdec(dmask.Value(), 2) << "\n" << "Veto Mask = " << hexdec(vmask.Value(), 2) << "\n" << "And Mask  = " << hexdec(amask.Value(), 2) << "\n" << "Or Mask   = " << hexdec(omask.Value(), 2) << "\n" << "Strobe period = "
        << hexdec(strobeperiod.Value(), 6) << "\n" << "Strobe length = " << hexdec(strobelength.Value(), 6) << "\n" << "Enable DUT Veto = " << hexdec(enabledutveto.Value(), 2) << "\n"

        //<< "Threshold voltage  1  = " << vthresh_values[0] << " V" << std::endl << "Threshold voltage  2  = " << vthresh_values[1] << " V" << std::endl << "Threshold voltage  3  = " << vthresh_values[2] << " V" << std::endl
    ...
    // Instantiate an instance of a caliceahcalbif
    SampicMonitor TLU(ConnectionFile.Value(), DeviceName.Value());
