...
class SampicTgDataCollector : public eudaq::DataCollector{
  public:
  SampicTgDataCollector(const std::string &name, const std::string &rc);
  ...
  static const uint32_t m_id_factory = eudaq::cstr2hash("SampicTgDataCollector");
private:
  std::mutex m_mtx_map;
...
namespace{
  auto dummy0 = eudaq::Factory<eudaq::DataCollector>::
    Register<SampicTgDataCollector, const std::string&, const std::string&>
    (SampicTgDataCollector::m_id_factory);
}

SampicTgDataCollector::SampicTgDataCollector(const std::string &name, const std::string &rc):
  DataCollector(name, rc){
}

void SampicTgDataCollector::DoConnect(eudaq::ConnectionSPC idx){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  m_conn_evque[idx].clear();
  m_conn_inactive.erase(idx);
}

void SampicTgDataCollector::DoDisconnect(eudaq::ConnectionSPC idx){
  std::unique_lock<std::mutex> lk(m_mtx_map);
...
void SampicTgDataCollector::DoConfigure(){
  m_noprint = 0;
  auto conf = GetConfiguration();
  if(conf){
    conf->Print();
    m_noprint = conf->Get("SAMPIC_DISABLE_PRINT", 0);
  }
}

void SampicTgDataCollector::DoReset(){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  m_noprint = 0;
  m_conn_evque.clear();
  m_conn_inactive.clear();
...
void SampicTgDataCollector::DoReceive(eudaq::ConnectionSPC idx, eudaq::EventSP evsp){
  std::unique_lock<std::mutex> lk(m_mtx_map);
  ...
  auto ev_sync = eudaq::Event::MakeUnique("SampicTg");
