...
void SrsProducer::RunLoop(){
  ...
  while (m_running) {
    for (size_t i = 0; i < m_srs->numFec(); ++i) {
      /*auto future = std::async(std::launch::async, &srs::SlowControl::readout, m_srs.get(), std::ref(m_frames_colls[i]), i, std::ref(m_running));
      if (m_timeout_sec > 0 && future.wait_for(std::chrono::seconds(m_timeout_sec)) == std::future_status::timeout) {
        m_srs->setReadoutEnable(false);
        m_running = false;
        EUDAQ_THROW("Timeout reached after "+std::to_string(m_timeout_sec)+" sec(s).");
      }*/
