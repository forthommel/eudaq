...
void SrsProducer::DoStartRun(){
  if (!m_sent_config) {
    EUDAQ_DEBUG("SYS register wrote ("+std::to_string(m_srs_config->GetBlock(0).size())+" words)");
    EUDAQ_DEBUG("APVAPP register wrote ("+std::to_string(m_srs_config->GetBlock(1).size())+" words)");
