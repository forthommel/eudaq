...
namespace {
  auto cstr_deser = Factory<Event>::Register<SrsConfig, Deserializer &>(
      SrsConfig::m_id_factory);
...
SrsConfig::SrsConfig() { SetType(m_id_factory); }
...
void SrsConfig::ConvertBlock(unsigned short reg,
                             const std::vector<uint8_t> &block8) {
  srs::words_t block32; // convert 8-bit to 32-bit words
  for (size_t i = 0; i < block8.size(); i += 4)
    block32.emplace_back(block8[i] + (block8[i + 1] << 8) +
                         (block8[i + 2] << 16) + (block8[i + 3] << 24));
  switch (reg) {
  case 0:
...
void SrsConfig::Print(std::ostream &os, size_t offset) const {
  os << std::string(offset, ' ') << "<SrsConfig>\n"
